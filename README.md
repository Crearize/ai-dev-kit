...
- macOS / Linux の場合: Bash
- Windows の場合: PowerShell

### 1. ai-dev-kit を取得

```bash
git clone https://github.com/Crearize/ai-dev-kit.git
```

### 2. プロジェクトに導入

導入したいプロジェクトのディレクトリに移動して、セットアップスクリプトを実行します。
...
# macOS / Linux
/path/to/ai-dev-kit/setup.sh init

# Windows (PowerShell)
C:\path\to\ai-dev-kit\setup.ps1 init
```
...
```bash
# macOS / Linux
/path/to/ai-dev-kit/setup.sh personal

# Windows (PowerShell)
C:\path\to\ai-dev-kit\setup.ps1 personal
```
...
### `setup.sh init` の対話フロー

セットアップスクリプトは対話形式で進みます。以下が実際の流れです。
...
| 選択肢 | 何が起きるか |
|--------|-------------|
| 1) Claude Code | `~/.claude/settings.json` に破壊的コマンドのブロックルールを追加（既存設定はバックアップ） |
...
├── .superpowers-version        # 同期中の superpowers バージョン
│
├── skills/                     # AI スキル定義
...
│
├── templates/                  # 生成ファイルの雛形
│   ├── CLAUDE.md.template      #   Claude Code 設定ファイル
│   ├── cursorrules.template    #   Cursor 設定ファイル
│   ├── settings.json.template  #   プロジェクト settings.json
│   ├── settings-global.json.template  # グローバル settings.json
│   ├── cursor-rule.mdc.template      # .mdc ルール変換テンプレート
│   └── PULL_REQUEST_TEMPLATE.md      # PR テンプレート
│
├── configs/                    # AI ツール別セットアップロジック
│   ├── claude-code/
│   │   ├── install.sh          #   Claude Code のインストール処理
│   │   └── README.md
│   └── cursor/
│       ├── install.sh          #   Cursor のインストール処理
│       └── README.md
│
├── scripts/                    # 内部ユーティリティ
│   ├── link-or-copy.sh         #   シンボリックリンク作成（失敗時はコピー）
│   ├── merge-settings.sh       #   JSON 設定ファイルのマージ
│   └── transform-skills.sh     #   superpowers スキルの変換・コピー
│
└── .github/workflows/
...
`setup.sh init` を実行すると、以下のファイルがプロジェクトに生成されます。
...
ai-dev-kit に新しい技術スタック（例: Python + Django）を追加する方法です。
...
`stacks/` ディレクトリに配置するだけで、セットアップスクリプトが自動的に検出します。
次回 `setup.sh init` を実行すると、選択肢に表示されます。
...
### Q: セットアップ後に ai-dev-kit リポジトリは必要ですか？

`setup.sh init` はファイルをコピーするため、セットアップ後は ai-dev-kit リポジトリがなくても動作します。ただし、以下の場合に再度必要になります:

- 新しいプロジェクトにセットアップしたい時
...
PowerShell スクリプト（`setup.ps1`）が用意されています。シンボリックリンクの代わりに NTFS ジャンクション（管理者権限不要）を使用し、失敗時はコピーにフォールバックします。
