...
async function doInit(options = {}) {
  ...
  try {
    ...
    if (skillChoice === 3) {
      ...
      if (availableSkills.length === 0) {
        ...
        if (skillInput.length === 1 && skillInput[0].toLowerCase() === 'all') {
          ...
          selectedProjectSkills = [];
          for (const idx of skillInput) {
            const num = parseInt(idx, 10);
            if (num >= 1 && num <= availableSkills.length) {
              selectedProjectSkills.push(availableSkills[num - 1]);
            } else {
              console.log(`  Warning: "${idx}" is not a valid skill number, skipping`);
            }
          }
        }
      }
    ...
    if (skillChoice === 0 || skillChoice === 1 || skillChoice === 3) {
      copyDirSync(
    ...
    } else if (skillChoice === 3 && selectedProjectSkills && selectedProjectSkills.length > 0) {
      copySelectedSkills(
        path.join(PACKAGE_ROOT, 'skills', 'project'),
        path.join(skillsDest, 'project'),
        selectedProjectSkills,
    ...
    if (!dryRun) {
      if (skillChoice === 3 && (!selectedProjectSkills || selectedProjectSkills.length === 0)) {
