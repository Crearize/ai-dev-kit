...
#### DRY原則の適用
- `init.js`: `setupClaudeCode()` と `setupCursor()` の共通処理を抽出
  - ディレクトリ作成
  - ルールファイルの配置
  - シンボリックリンク作成
- `personal.js`: `setupClaudeCodeGlobal()` と `setupCursorGlobal()` の共通処理も同様に抽出
- 共通処理を内部関数として各ファイル内に定義
...
| 自動マージ | ラベルベースの自動マージを導入（下記 4.1.1 参照） |

#### 4.1.1 superpowers同期PRの自動マージ

**方式**: GitHub Auto-merge + bypass list + ラベル制限

**手順**:
1. リポジトリ設定で Auto-merge を有効化
2. ブランチ保護ルールの bypass list に `github-actions[bot]` を追加
3. `sync-superpowers.yml` 内で:
   - PRに専用ラベル `auto-merge-superpowers` を付与
   - `gh pr merge --auto --squash` を実行
4. ラベルが付いたPRのみ自動マージ対象となる

**安全性**:
- bypass list は `github-actions[bot]` のみ
- 自動マージはワークフロー内の `gh pr merge` でのみ実行
- ラベルを手動で付けても、`github-actions[bot]` がPR作成者でなければbypassされない
...
| Auto-merge方式（bypass + ラベル制限） | シンプルかつ実質的に安全。superpowers同期PRのみが対象となる |
